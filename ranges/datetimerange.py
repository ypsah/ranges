...
@total_ordering
class DatetimeRange(Hashable, Sequence[datetime], Set[datetime]):
    """
    ...
    def __init__(self, start: datetime, stop: datetime, step: timedelta, *args, **kwargs):
        ...
        self._stop = stop
        self._step = step
        self._len = max(0, ceil((stop - start) / step))
    ...
    def __str__(self) -> str:
        return f"{self.__class__.__name__}({self.start!s}, {self.stop!s}, {self.step!s})"
    ...
    def __reversed__(self) -> DatetimeRange:
        if not self:
            return DatetimeRange(self.stop, self.start, -self.step)
        return DatetimeRange(self[-1], self.start - self.step, -self.step)
